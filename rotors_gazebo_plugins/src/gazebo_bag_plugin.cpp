...
void GazeboBagPlugin::StartRecording() {
  ...
  // Subscriber to Image Message.
  image_sub_ = it_->subscribe("/cam0/image_raw", 1, &GazeboBagPlugin::ImageCallback, this);
...
void GazeboBagPlugin::ImageCallback(const sensor_msgs::ImageConstPtr& image_msg) {
  common::Time now = world_->GetSimTime();
  ros::Time ros_now = ros::Time(now.sec, now.nsec);
  writeBag("/cam0/image_raw", ros_now, image_msg);
}

void GazeboBagPlugin::StateEstimateCallback(const nav_msgs::OdometryConstPtr& state_est_msg) {
  common::Time now = world_->GetSimTime();
  ros::Time ros_now = ros::Time(now.sec, now.nsec);
