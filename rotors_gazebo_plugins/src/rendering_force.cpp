...
RenderingForce::RenderingForce(const std::string &_name, rendering::VisualPtr _parent_vis)
...
void RenderingForce::Load()
{
  ...
  selected_material_ = "Gazebo/Purple";
  ...
  for (int i = 0; i < force_visuals_.size(); i++) {
    force_visuals_[i]->SetMaterial(selected_material_);
    force_visuals_[i]->GetSceneNode()->setInheritScale(false);
  }
  ...
  this->Resize();
  this->UpdateForcesVisual();
}

void RenderingForce::SetForce(const math::Vector3 &force_vector) {
  force_vector_ = force_vector;
...
void RenderingForce::UpdateForcesVisual() {
  if (force_visuals_.empty()) {
    gzwarn << "No force visuals.\n";
    return;
  }
...
void RenderingForce::Resize() {
  if (!this || force_visuals_.empty()) {
    gzwarn << "RenderingForce is incomplete.\n";
    return;
  }
...
math::Quaternion RenderingForce::QuaternionFromVector(const math::Vector3 &vec) {
  double roll = 0;
